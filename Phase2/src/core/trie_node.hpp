...
struct TrieNode {
    ...
    // Hit-count / popularity for this word. Only meaningful when
    // isEndOfWord is true. Lives directly on the node (rather than in a
    // side map) because it's intrinsic per-word metadata the Trie itself
    // needs for ranked autocomplete -- see Trie::autocomplete().
    long frequency = 0;
